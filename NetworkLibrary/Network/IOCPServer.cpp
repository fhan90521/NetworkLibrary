	...
	while (1)
	{
		...
		else
		{
			...
			else if (pOverlapped == (LPOVERLAPPED)PROCESS_JOB)
			{
				...
				SharedPtr<JobQueue> jobQ = ((JobQueue*)pSession)->_selfPtr;
...
bool IOCPServer::ServerControl()
{
	...
	if (_kbhit())
	{
		...
		{
			bControlMode = true;
			Log::LogOnConsole(Log::SYSTEM_LEVEL, "Control Mode : Press Q - Quit\n");
			Log::LogOnConsole(Log::SYSTEM_LEVEL, "Control Mode : Press L - Key Lock\n");
		}

		if (bControlMode && ('l' == ControlKey || 'L' == ControlKey))
		{
			Log::LogOnConsole(Log::SYSTEM_LEVEL, "Control Lock... Press U - Control Unlock\n");
			bControlMode = false;
		}
